...
export class ProductoController {
    ...
    @Get()
    getAll(@Res() response) {
        this.productService.getAll().then(( productos: Producto[] ) => {
    ...
    get(
        ...
        this.productService.get(id).then(( producto: Producto ) => {
            if ( producto ) {
                response.status(HttpStatus.OK)
                .json(new CResponse(Status.OK, 'Exito', this.authService.token, producto));
            } else {
                response.status(HttpStatus.OK)
                .json(new CResponse(Status.NOT_FOUND_RECORD, `La producto con ID: ${  id.toString() } no existe`));
            }
        }).catch((error) => {
            response.status(HttpStatus.INTERNAL_SERVER_ERROR)
